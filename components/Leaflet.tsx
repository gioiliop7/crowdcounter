...
import { Settings, Map, Users, BarChart2 } from "lucide-react";
...
export default function Leaflet() {
  ...
  useEffect(() => {
    const polygonArray = polygonsData.map(
      (polygon: PolygonData, index: number) => {
        ...
        const leafletPolygon = L.polygon(polygon.coordinates as L.LatLngExpression[], {
          color,
          fillOpacity: 0.6,
          weight: 2,
        });
    ...
    setTotalPopulation(
      polygonArray.reduce((acc, item) => acc + item.population, 0)
    );
  }, [density]);
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 min-h-screen p-4 bg-gradient-to-br from-gray-900 to-red-900 text-white">
      <div className="lg:col-span-3 rounded-xl overflow-hidden shadow-2xl relative h-[70vh] lg:h-auto">
        <MapContainer
        ...
        <div className="mt-4">
          <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-red-700 to-gray-700 p-6 shadow-lg">
            ...
            <div className="mt-1 text-[10px] opacity-75">
              * Η εκτίμηση βασίζεται σε δεδομένα πληθυσμού και ενδέχεται να
              έχει αποκλίσεις. Σε συνθήκες πορείας: - 1 άτομο: Τυπική κίνηση,
              πιθανό στην αρχή ή στο τέλος της πορείας. - 2-3 άτομα: Μικρή
              συγκέντρωση, διάσπαρτες ομάδες ή άτομα που κινούνται
              περιμετρικά. - 4-5 άτομα: Πυκνότερη συγκέντρωση, συνήθως σε
              βασικά σημεία της πορείας ή κοντά σε ηχητικά μέσα. Ο συνολικός
              πληθυσμός μπορεί να διαφέρει ανάλογα με την τοποθεσία, τη
              διάρκεια και τη δυναμική της συγκέντρωσης.
            </div>
          </div>
        </div>
